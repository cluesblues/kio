...
public:
    ...
    void errorCopyingFilesHelper(const QList<CopyInfo>::Iterator &it, RenameDialog_Result result,
                                 const QUrl &newUrl);
...
void CopyJobPrivate::slotResultConflictCreatingDirs(KJob *job)
{
    ...
    QObject::connect(q->askUserActionInterface(), &KIO::AskUserActionInterface::askUserRenameResult,
        ...
        QObject::disconnect(q->askUserActionInterface(), &KIO::AskUserActionInterface::askUserRenameResult,
                            q, nullptr);
    ...
    q->askUserActionInterface()->askUserRename(q, i18n("++++Folder Already Exists"),
...
void CopyJobPrivate::slotResultErrorCopyingFiles(KJob *job)
{
    ...
    if ((m_conflictError == ERR_FILE_ALREADY_EXIST)
        ...
        const QString caption = !isDir ? i18n("conflict File Already Exists") : i18n("conflict Already Exists as Folder");

        QObject::connect(q->askUserActionInterface(), &KIO::AskUserActionInterface::askUserRenameResult,
                         q, [q, this, it](RenameDialog_Result result, const QUrl &newUrl, KJob *parentJob) {
            // If there are multiple CopyJobs, only react to the signal from the dialog invoked
            // by _this_ job
            if (parentJob != q) {
                return;
            ...
            errorCopyingFilesHelper(it, result, newUrl);
        });

        q->askUserActionInterface()->askUserRename(q, caption,
                                                   (*it).uSource, (*it).uDest, options,
                                                   (*it).size, destsize,
                                                   (*it).ctime, destctime,
                                                   (*it).mtime, destmtime);
        ...
        if (job->error() == ERR_USER_CANCELED) {
            res = Result_Cancel;
        } else if (!q->askUserActionInterface()) {
            q->Job::slotResult(job);   // will set the error and emit result(this)
            return;
        } else {
            ...
            QObject::connect(q->askUserActionInterface(), &KIO::AskUserActionInterface::askUserSkipResult,
                ...
                QObject::disconnect(q->askUserActionInterface(), &KIO::AskUserActionInterface::askUserSkipResult,
                                    q, nullptr);
                errorCopyingFilesHelper(it, result, QUrl() /* no new url in skip */);
            });

            q->askUserActionInterface()->askUserSkip(q, options, job->errorString());
            return;
        }
    }

    errorCopyingFilesHelper(it, res, QUrl());
}

void CopyJobPrivate::errorCopyingFilesHelper(const QList<CopyInfo>::Iterator &it,
...
void CopyJobPrivate::slotResultRenaming(KJob *job)
{
    ...
    if (err) {
        ...
        auto couldNotRename = [q, this, dest]() {
        ...
        if (err == ERR_DIR_ALREADY_EXIST ||
            ...
            if ((isDir && m_bAutoSkipDirs) || (!isDir && m_bAutoSkipFiles)) {
                ...
                const QString caption = err != ERR_DIR_ALREADY_EXIST ? i18n("debug File Already Exists") : i18n("debug Already Exists as Folder");
